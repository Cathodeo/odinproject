def caesar_cipher(sentence, factor)
  ...
  shifted_numbers = num_values.map do |number|
    case number 
    when 65..(90 - factor) then number = number + factor
    when (91 - factor)..90 then number = (number - 26) + factor  
    when 97..(122 - factor) then number = number + factor
    when (123 - factor)..122 then number = (number - 26) + factor
    else number = number
    end
  end 
